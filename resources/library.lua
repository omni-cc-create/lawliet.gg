...
local function GetConfigs(PFName)
	if not isfolder(PFName) then
		makefolder(PFName)
	end
	if not isfolder(PFName .. "\\Configs") then
		makefolder(PFName .. "\\Configs")
	end
	if not isfile(PFName .. "\\DefaultConfig.txt") then
...
local function ConfigsToList(PFName)
	...
	for Index, Config in pairs(listfiles(PFName .. "\\Configs") or {}) do
		...
		Configs[Index] = { Name = Config, Mode = "Button", Value = Config == DefaultConfig }
	end
	return Configs
end
...
local function InitToolTip(Parent, ScreenAsset, Text)
	Parent.MouseEnter:Connect(function()
		...
		ScreenAsset.ToolTip.Size =
			UDim2.new(0, ScreenAsset.ToolTip.TextBounds.X + 2, 0, ScreenAsset.ToolTip.TextBounds.Y + 2)
...
local function InitWindow(ScreenAsset, Window)
	...
	WindowAsset.TabButtonContainer.ListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
		WindowAsset.TabButtonContainer.CanvasSize =
			UDim2.new(0, WindowAsset.TabButtonContainer.ListLayout.AbsoluteContentSize.X, 0, 0)
	end)
	RunService.RenderStepped:Connect(function()
		if WindowAsset.Visible then
			ScreenAsset.ToolTip.Position =
				UDim2.new(0, UserInputService:GetMouseLocation().X + 5, 0, UserInputService:GetMouseLocation().Y - 5)
		end
	end)
	RunService.RenderStepped:Connect(function()
	...
	function Window:SetColor(Color)
		if Color.R < 5 / 255 and Color.G < 5 / 255 and Color.B < 5 / 255 then
	...
	function Window:Toggle(Boolean)
		...
		if not Debug then
			RunService:SetRobloxGuiFocused(Window.Enabled and Window.Flags["UI/Blur"])
		end
	...
	function Window:Watermark(Watermark)
		...
		ScreenAsset.Watermark.Size =
			UDim2.new(0, ScreenAsset.Watermark.Title.TextBounds.X + 6, 0, ScreenAsset.Watermark.Title.TextBounds.Y + 6)
		MakeDraggable(ScreenAsset.Watermark, ScreenAsset.Watermark, function(Position)
			Window.Flags[Watermark.Flag] = { Position.X.Scale, Position.X.Offset, Position.Y.Scale, Position.Y.Offset }
		end)
		...
		function Watermark:SetTitle(Text)
			...
			ScreenAsset.Watermark.Size = UDim2.new(
				0,
				ScreenAsset.Watermark.Title.TextBounds.X + 6,
				0,
				ScreenAsset.Watermark.Title.TextBounds.Y + 6
			)
		end
		function Watermark:SetValue(Table)
			...
			ScreenAsset.Watermark.Position = UDim2.new(Table[1], Table[2], Table[3], Table[4])
		end
	...
	function Toggle:Keybind(Keybind)
		...
		UserInputService.InputBegan:Connect(function(Input, GameProcessed)
			...
			if Keybind.Mouse then
				...
				if
					Keybind.WaitingForBind and Input.UserInputType == Enum.UserInputType.MouseButton1
					or Keybind.WaitingForBind and Input.UserInputType == Enum.UserInputType.MouseButton2
					or Keybind.WaitingForBind and Input.UserInputType == Enum.UserInputType.MouseButton3
				then
					ToggleAsset.Keybind.Text = "[ " .. Key .. " ]"
...
local function InitSlider(Parent, ScreenAsset, Window, Slider)
	...
	local function AttachToMouse(Input)
		local XScale = math.clamp(
			(Input.Position.X - SliderAsset.Background.AbsolutePosition.X) / SliderAsset.Background.AbsoluteSize.X,
			0,
			1
		)
...
local function InitTextbox(Parent, ScreenAsset, Window, Textbox)
	...
	TextboxAsset.Title:GetPropertyChangedSignal("TextBounds"):Connect(function()
		TextboxAsset.Size =
			UDim2.new(1, 0, 0, (TextboxAsset.Title.TextBounds.Y + 2) + (TextboxAsset.Background.Input.TextBounds.Y + 2))
	end)
...
local function InitDropdown(Parent, ScreenAsset, Window, Dropdown)
	...
	DropdownAsset.Title:GetPropertyChangedSignal("TextBounds"):Connect(function()
		...
		DropdownAsset.Size =
			UDim2.new(1, 0, 0, DropdownAsset.Title.Size.Y.Offset + DropdownAsset.Background.Size.Y.Offset)
	end)
	...
	ColorpickerAsset.MouseButton1Click:Connect(function()
		if not PaletteAsset.Visible then
			...
			PaletteRender = RunService.RenderStepped:Connect(function()
				...
				PaletteAsset.Position = UDim2.new(
					0,
					(ColorpickerAsset.Color.AbsolutePosition.X - PaletteAsset.AbsoluteSize.X) + 20,
					0,
					ColorpickerAsset.Color.AbsolutePosition.Y + 52
				)
			end)
		else
			PaletteRender:Disconnect()
		...
		if Input.UserInputType == Enum.UserInputType.MouseButton1 then
			...
			SVRender = RunService.RenderStepped:Connect(function()
				...
				local ColorX = math.clamp(
					Mouse.X - PaletteAsset.SVPicker.AbsolutePosition.X,
					0,
					PaletteAsset.SVPicker.AbsoluteSize.X
				) / PaletteAsset.SVPicker.AbsoluteSize.X

				local ColorY = math.clamp(
					Mouse.Y - (PaletteAsset.SVPicker.AbsolutePosition.Y + 36),
					0,
					PaletteAsset.SVPicker.AbsoluteSize.Y
				) / PaletteAsset.SVPicker.AbsoluteSize.Y
	...
	PaletteAsset.Hue.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 then
			...
			HueRender = RunService.RenderStepped:Connect(function()
				...
				local Mouse = UserInputService:GetMouseLocation()
				local ColorX = math.clamp(
					Mouse.X - PaletteAsset.Hue.AbsolutePosition.X,
					0,
					PaletteAsset.Hue.AbsoluteSize.X
				) / PaletteAsset.Hue.AbsoluteSize.X
	...
	PaletteAsset.Alpha.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 then
			...
			AlphaRender = RunService.RenderStepped:Connect(function()
				...
				local Mouse = UserInputService:GetMouseLocation()
				local ColorX = math.clamp(
					Mouse.X - PaletteAsset.Alpha.AbsolutePosition.X,
					0,
					PaletteAsset.Alpha.AbsoluteSize.X
				) / PaletteAsset.Alpha.AbsoluteSize.X
...
function Bracket:Window(Window)
	...
	function Window:Tab(Tab)
		...
		function Tab:AddConfigSection(PFName, Side)
			local ConfigSection = Tab:Section({ Name = "Configs", Side = Side })
			do
				...
				ConfigDropdown = ConfigSection:Dropdown({ Name = "List", IgnoreFlag = true, List = ConfigList })
				...
				local ConfigDivider = ConfigSection:Divider({
					Text = DefaultConfig
		...
		function Tab:Section(Section)
			...
			function Section:Keybind(Keybind)
				...
				Keybind.Mouse = GetType(Keybind.Mouse, false, "boolean")
				Keybind.Callback = GetType(Keybind.Callback, function() end, "function")
				Keybind.Blacklist = GetType(Keybind.Blacklist, {
					"W",
					"A",
					"S",
					"D",
...
function Bracket:Notification(Notification)
	...
	NotificationAsset.Size = UDim2.new(
		0,
		GetLongest(NotificationAsset.Title.TextBounds.X, NotificationAsset.Description.TextBounds.X) + 24,
		0,
...
function Bracket:Notification2(Notification)
	...
	NotificationAsset.Main.Size =
		UDim2.new(0, NotificationAsset.Main.Title.TextBounds.X + 10, 0, NotificationAsset.Main.Title.TextBounds.Y + 6)
	NotificationAsset.Size = UDim2.new(0, 0, 0, NotificationAsset.Main.Size.Y.Offset + 4)
